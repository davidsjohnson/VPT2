...
def data_gen(hs, feature_func, pixels_per_cell=(8,8), cells_per_block=(3,3), img_size=(128, 128)):
    ...
    for i, (lh_dmap, lh_label, rh_dmap, rh_label, filename) in bar(enumerate(hgen)):
            ...
            x_lh = feature_func(lh_dmap, pixels_per_cell=pixels_per_cell, block_norm=None, img_size=img_size, pad_img=False)
            x_rh = feature_func(rh_dmap, pixels_per_cell=pixels_per_cell, block_norm=None, img_size=img_size, pad_img=False)
    ...
    for f in features:
        for c in cell_sizes:
                ...
                if f == honv:
                    ...
                    fname = "hog"
                feature_type = "f_{}-c_{}-b_none".format(fname, c[0])

                data_type = "train"

                hs = get_handstream(M, radius, participants)
                print("Extracting Features:", fname, "- Pixels per Cell:", c, "- No Block Norm",)
                X_lh, y_lh, X_rh, y_rh, filenames = data_gen(hs, f, pixels_per_cell=c, )
                print("Saving Data...")
