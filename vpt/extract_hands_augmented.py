...
from vpt.streams.augmented_stream import *
...
def init_hg(folders, testing_p, annotation_file, offset_gen, feature_gen,
                        M, radius, n_samples=750, ftype=".npz"):

    annotations = load_annotations(annotation_file)
    fs = AugmentedStream(folders, annotations=annotations, ignore=True)
...
def extract_hands(hg):
    ...
    for i, (lh, rh) in enumerate(hgen):
        ...
        if lh.label() != None and rh.label() != None:
            ...
            plt.figure()
            plt.subplot(121)
            plt.imshow(lh.get_hand_img())
            plt.subplot(122)
            plt.imshow(rh.get_hand_img())
            plt.show()
...
if __name__ == '__main__':
    ...
    posture_folders = {p: os.path.join("data/rdf/training-rand", p) for p in participants}
    ...
    print("Done")
    print("Saving Hands...")
    np.savez_compressed("data/hands/hands-augmented-M{:d}-rad{:0.2f}-p{:s}".format(M, radius, s.participant), lh_dmaps=lh_dmaps, y_lh=y_lh, rh_dmaps=rh_dmaps, y_rh=y_rh, filenames=filenames )
    print("Done")